...
describe("CrystalExchangeSystem", function()
    ...
    it("sells equipped items for crystals", function()
        local ItemSystem = require("src.ItemSystem")
        local items = ItemSystem.new()
        items:equip("Hat", {name = "Mage Hat", slot = "Hat", rarity = "A"})
        local ok = CrystalExchangeSystem:sellEquippedItem(items, "Hat")
        assert.is_true(ok)
        assert.is_nil(items.slots.Hat)
        assert.equals(15, GachaSystem.crystals)
    end)
end)
