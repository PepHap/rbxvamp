...
describe("ItemSystem", function()
    ...
    it("serializes and loads correctly", function()
        ...
        items:addItem({name = "Ring", slot = "Ring"})
        local data = items:toData()
        local restored = ItemSystem.fromData(data)
        assert.equals("Cap", restored.slots.Hat.name)
        assert.equals(1, #restored.inventory)
        assert.equals("Ring", restored.inventory[1].name)
    end)
end)
