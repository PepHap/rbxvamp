...
describe("ItemSalvageSystem", function()
    ...
    it("salvages an equipped item from a slot", function()
        local items = ItemSystem.new()
        items:equip("Hat", {name="Cap", slot="Hat", rarity="B"})
        local ok = SalvageSystem:salvageFromSlot(items, "Hat")
        assert.is_true(ok)
        assert.is_nil(items.slots.Hat)
        -- B rarity gives 2 gold and 1 crystal at level 1
